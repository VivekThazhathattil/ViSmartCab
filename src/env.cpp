...
Env::Env() {
	...
	G.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
	G.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE/2;

	B.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*3 + GRID_SIZE/2;
	B.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;

	Y.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE/2;
	Y.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
		...
		for (int j = 0; j < numPassengerStates; j++)
			for (int b = 0; b < numCabYStates; b++)
				for( int a = 0; a < numCabXStates; a++)
					encodeArr[a +\
						numCabXStates * b +\
						numCabXStates * numCabYStates * j +\
						numCabXStates * numCabYStates * numPassengerStates * k\
					] = 0;
}
...
void Env::initializeRewardTable(){
	...
	passenger.setPassengerStatus(false);// getNextState may cause passenger status to vary
...
void Env::saveRewardTableToFile(std::string fileName){
	...
	for (int i = 0; i < NUM_GRIDS_X * NUM_GRIDS_Y * NUM_PASSENGER_STATES * NUM_DEST_STATES; i++)
	for (int j = 0; j < NUM_ACTIONS; j++){
		decode((i)%(NUM_GRIDS_X * NUM_GRIDS_Y * NUM_PASSENGER_STATES * NUM_DEST_STATES),cabI0,cabJ0,psg_idx0,dest_idx0);
