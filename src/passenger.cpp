...
void Passenger::setSpecificSpawnAndDropPosition(int s, int d) {
	switch(s){
		case 0:
			...
			pickUpGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE/2;
			pickUpGridPos.y = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE/2;
			break;
		case 1:
			isPassengerInCab = false;
			pickUpCode = 'G';
			pickUpPos = getPosFromLocCode(s);
			pickUpGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
			pickUpGridPos.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE/2;
			break;
		case 2:
			isPassengerInCab = false;
			pickUpCode = 'B';
			pickUpPos = getPosFromLocCode(s);
			pickUpGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*3 + GRID_SIZE/2;
			pickUpGridPos.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
			break;
		case 3:
			isPassengerInCab = false;
			pickUpCode = 'Y';
	...
	switch(d){ // didn't implement check for different s and d, assuming the user invokes correctly
		case 0:
			...
			dropOffGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE/2;
			dropOffGridPos.y = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE/2;
			break;
		case 1:
			dropOffCode = 'G';
			dropOffPos = getPosFromLocCode(d);
			dropOffGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
			dropOffGridPos.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE/2;
			break;
		case 2:
			dropOffCode = 'B';
			dropOffPos = getPosFromLocCode(d);
			dropOffGridPos.x = (WINDOW_SIZE_X - NUM_GRIDS_X*GRID_SIZE)/2 + GRID_SIZE*3 + GRID_SIZE/2;
			dropOffGridPos.y = (WINDOW_SIZE_Y - NUM_GRIDS_Y*GRID_SIZE)/2 + GRID_SIZE*4 + GRID_SIZE/2;
			break;
		case 3:
			dropOffCode = 'Y';
			dropOffPos = getPosFromLocCode(d);
...
bool Passenger::getPassengerStatus(){
	return isPassengerInCab;
}

void Passenger::setPassengerStatus(bool val){
	isPassengerInCab = val;
}
